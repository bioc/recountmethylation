...
up to 193 Gb of disk space to store a single database file. Files pair 
...
sm$version <- ifelse(grepl("0-0-1", sm$filename), "v0.0.1",
                     ifelse(grepl("0-0-2", sm$filename), "v0.0.2",
                            ifelse(grepl("0-0-3", sm$filename), "v0.0.3", "NA")))
sm$`size (bytes)` <- unlist(lapply(sm$`size (bytes)`, function(si){
  val <- unlist(strsplit(si, ";"))
  val <- paste0(round(as.numeric(gsub(".* = ", "", val[1]))/1e9,2), "Gb")
  val
}))
sm <- sm[rev(order(sm$version)),]
